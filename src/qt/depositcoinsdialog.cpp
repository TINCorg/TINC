...
#include "ui_depositcoinsdialog.h"
...
DepositCoinsDialog::DepositCoinsDialog(QWidget *parent) :
    QDialog(parent),
...
void DepositCoinsDialog::setModel(WalletModel *model)
{
    this->model = model;
...
void DepositCoinsDialog::on_sendButton_clicked()
{
    ...
    if (model->getOptionsModel()->getCoinControlFeatures()) // coin control enabled
        prepareStatus = model->prepareTransaction(currentTransaction, termDepositConfirmQuestion, termDepositBlocks, CoinControlDialog::coinControl);
    else
...
void DepositCoinsDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance, std::vector<COutput> termDepositCoins)
{
...
void DepositCoinsDialog::updateGlobalFeeVariables()
{
...
void DepositCoinsDialog::coinControlClipboardPriority()
{
...
void DepositCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid TINC address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
