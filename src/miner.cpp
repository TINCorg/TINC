...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    txNew.vout[0].scriptPubKey = scriptPubKeyIn;
		...
		CBitcoinAddress address(DEV_ADDRESS);
		txNew.vout.resize(2);
		txNew.vout[1].scriptPubKey = GetScriptForDestination(CBitcoinAddress(address).Get());
...

    {
        ...
        txNew.vout[1].nValue = GetDevCoin(txNew.vout[0].nValue);
...
static bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TINCMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("TINCMiner: ProcessNewBlock, block not accepted");
...
void static BitcoinMiner(CWallet *pwallet, uint32_t minerI, uint32_t minerN, int nThreads)
{
        ...
        while (true) {
            ...
            if(ma!=""){
            	if(validateAddress(ma)) {
                    LogPrintf("TINCMiner: Mining to User supplied address %s\n",ma);
                    pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithAddress(ma));
                }else{
                    LogPrintf("TINCMiner: WARNING! User supplied address is invalid, defaulting to keypool address.\n");
                    pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithKey(reservekey));
                }
            }else{
                LogPrintf("TINCMiner: Mining to keypool address.\n");
                pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithKey(reservekey));
            }
            if (!pblocktemplate.get())
            {
                LogPrintf("TINCMiner: ERROR! Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running TINCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("TINCMiner terminated\n");
    ...
    {
        LogPrintf("TINCMiner runtime error: %s\n", e.what());
        //aligned_free(scratchpad);
        fGenerate = false;
        return;
    }
