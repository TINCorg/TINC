...
std::string initRateTable() {
    ...
    bonusTable[0] = 1;
    bonusTable[0] = bonusTable[0] << 58;		// 54

    //Interest rate on each block 1+(1/2^22)
    for(int i=1;i<ONEYEARPLUS1;i++){
        rateTable[i]  = rateTable[i-1]  + (rateTable[i-1]  >> 22);		// 5%
        bonusTable[i] = bonusTable[i-1] + (bonusTable[i-1] >> 20);		// 20%
...
CAmount GetInterest(CAmount nValue, int outputBlockHeight, int valuationHeight, int maturationBlock){
    ...
    int blocks=0;		// regular deposits have no interest on TINC

    //Term deposits may have up to 1 year of interest (1 month on TINC)
    if(maturationBlock>0){
        //blocks=std::min(ONEYEAR,valuationHeight-outputBlockHeight);
        blocks=std::min(THIRTYDAYS,maturationBlock-outputBlockHeight);		// TINC doesn't use valuation
    ...
    if(maturationBlock>0){
        ...
        */
        // TINC: no interest on less than 1 days term deposits
